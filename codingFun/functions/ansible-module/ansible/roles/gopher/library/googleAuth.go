...
func DisableGoogleAuth() {
	fmt.Println("Disabling TFA")
	// copy back up files and restart sshd :) simple
	//                                1                                   2                                   3
	//DisableCommands = []string{"systemctl restart sshd", "yum remove google-authenticator -y"}
	...
	fmt.Println("Uninstalling google-authenticator")
	_, err = exec.Command("yum", "remove", "google-authenticator", "-y").Output()
	if err != nil {
		fmt.Println("Error while starting sshd daemon")
		os.Exit(1)
	}
